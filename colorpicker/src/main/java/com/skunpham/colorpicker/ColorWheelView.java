...
public class ColorWheelView extends FrameLayout implements ColorObservable, Updatable {
    ...
    public void update(MotionEvent event) {
        ...
        if (!onlyUpdateOnTouchEventUp || isTouchUpEvent) {
            int nowColor = getColorAtPoint(x, y);
            emitter.onColor(nowColor, true, isTouchUpEvent);
            updateSelector(x, y, nowColor);
        } else {
            updateSelector(x, y, currentColor);
        }
    }
    ...
    public void setColor(int color, boolean shouldPropagate) {
        ...
        updateSelector((float) (r * Math.cos(radian) + centerX), (float) (-r * Math.sin(radian) + centerY), color);
        currentColor = color;
    ...
    private void updateSelector(float eventX, float eventY, int color) {
        float x = eventX - centerX;
        float y = eventY - centerY;
        double r = Math.sqrt(x * x + y * y);
        if (r > radius) {
